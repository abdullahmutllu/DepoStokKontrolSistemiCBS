...
import { WarehouseScene } from "@/features/three/WarehouseScene";
import { DetailPanel } from "@/features/three/DetailPanel";
import { OCCUPANCY_COLORS, OCCUPANCY_LEGEND } from "@/features/three/occupancy";
  ...
  return (
    <div>
      ...
      <div className="relative h-[520px] overflow-hidden rounded-md border border-ink-600 bg-ink-950">
        ...
        <DetailPanel />
        {/* Camera presets */}
        <div className="absolute left-3 top-3 flex gap-0.5 rounded border border-ink-600 bg-ink-900/90 p-0.5 backdrop-blur">
          {presets
            .filter((p) => p.id !== "reset")
            .map((p) => (
              <button
